...
contract WhenCheapSession is ReentrancyGuard, Pausable {
    ...
    function execute(address to, uint256 value, bytes calldata data) external payable nonReentrant whenNotPaused {
        ...
        uint256 totalFee = feeForAmount(value);
        uint256 agentFee = agentFeeForAmount(value);
        uint256 treasuryFee = treasuryFeeForAmount(value);
        uint256 net = value - totalFee;
        ...
        if (agentFee > 0) {
            ...
            if (!agentOk) {
                ...
                emit FeeCollected(agentAddress, agentFee, value, "agent");
            }
        }
        ...
        if (treasuryFee > 0) {
            ...
            if (!treasuryOk) {
                ...
                emit FeeCollected(treasury, treasuryFee, value, "treasury");
            }
        }
    ...
    function executeBatch(Call[] calldata calls) external payable nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < calls.length; i++) {
            uint256 totalFee = feeForAmount(calls[i].value);
            uint256 agentFee = agentFeeForAmount(calls[i].value);
            uint256 treasuryFee = treasuryFeeForAmount(calls[i].value);
            uint256 net = calls[i].value - totalFee;
            ...
            if (agentFee > 0) {
                ...
                if (!agentOk) {
                    ...
                    emit FeeCollected(agentAddress, agentFee, calls[i].value, "agent");
                }
            }
            ...
            if (treasuryFee > 0) {
                ...
                if (!treasuryOk) {
                    ...
                    emit FeeCollected(treasury, treasuryFee, calls[i].value, "treasury");
                }
            }
